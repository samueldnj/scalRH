...
of fishing mortality rate [@collie2001biological].
...
derived, but the technical interactions are not considered, then indicental 
catch  will often lead to sub-optimal fishery outcomes 
[@ono2017management]. For example, a choke effect could occur if 
catch limits are set to meet target fishing mortality rates according to 
multi-species reference points that ignore technical interactions, where 
easier to catch species have their catch limits fulfilled faster and thereby
limit fishing opportunities species that have higher catch limits
[@baudron2015adverse].
...
bio-economic models for the derivation of net economic benefit 
...
those bio-economic models, and ecological interactions are sometimes ignored 
Costs of fishing and landed values are readily added 
...
to quota transfers is ignored, using catch shares to manage a fishery is 
similar to a "sole operator". 
...
optimal economic yield, partly because stochasticity is often ignored
for MEY calculations in the economic literatures [@sanchirico2007sustainable; 
...
price of is the same, regardlessof the quantity landed or the annual quota. 
...
In this paper, we investigated the differences between steady-state 
and dynamic multi-species MSY and MEY reference points for a spatially 
heterogeneous flatfish fishery targeting three species of right-eyed 
flounders (*Pleuronectidae Spp.*) - Dover sole (*Microstomus pacificus*), 
English sole (*Parophrys vetulus*), and (Southern) Rock sole (*Lepidopsetta 
bilineata*) - fished by a common trawl fishery off the coast of British 
Columbia, Canada. Steady-state multi-species maximum yield reference 
points incorporated technical interactions of the three species to relate 
species specific yield curves to a common trawl effort [@pikitch1987use], 
...
deriving net economic benefits from those steady state solutions. For dynamic 
optima, stochastic optimisation was used to solve for effort levels 
maximising yield and discounted profit under 16 stochastic scenarios, which 
varied in whether recruitment process deviations were correlated, market 
price fluctuations were corelated, and whether the total amount of effort 
available in the fishery was fixed or allowed to gradually change from
year to year. Finally, we discuss the optimal fishing efforts and associated 
harvest rates under each scenario, and the implications for biological and 
economic fishery outcomes when managed under objectives that target MSY 
and MEY.

Multi-species MSY and MEY reference points were estimated by fitting
a hierarchical, multi-species, age-structured stock assessment 
model to data from the BC flatfish fishery. Some flatfish stocks lacked 
