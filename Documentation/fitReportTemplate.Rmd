...
dataSummCaption <- "Summary of data used to fit hierSCAL."
```
...
plotScaledIdxGrid( reports = reports )
```

```{r, idxResidsAll, echo = FALSE, fig.cap = IdxResCap }
plotIdxResidsGrid( reports = reports )
```
...
```{r makeSpeciesChild, echo = FALSE, results = "asis"}
for( sIdx in 1:nS )
{
  cat(paste("## ", species[sIdx], "\n",sep = "")) 

  childDoc <- knitr::knit_child(here::here("Documentation","speciesChild.Rmd"),options = list(results = "asis"),quiet = TRUE)
...
# Data Summary
```{r, plotDataSummary, echo = FALSE, message = FALSE, fig.cap = dataSummCaption }
plotDataSummary(reports)
```
