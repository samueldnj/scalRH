...
appendix: false
abstract: Fisheries stock assessments are typically performed for a single species or stock at a time. Not all stocks have the same available data (e.g. missing age structures), due to low fishing effort or sampling effort, so go un-assessed or get aggregated with other management units, despite heterogeneity in abundance or biological data. Modern hierarchical multi-species and multi-stock assessment models are an alternative option for dealing with data limitations in spatially diverse fisheries. For example, biological information can be shared between stocks within a species, and environmental effects may be assumed to be shared between species that inhabit the same stock area. In this paper, we present a multi-species, multi-stock, age structured integrated assessment model, able to be fit to abundance, age, and length data for 5 flatfish species in 3 stock areas on the coast of British Columbia. For 2 species, at least one stock area is missing age composition data, so length compositions are used in their place. To share information between stocks, hierarchical shrinkage priors are applied to catchability and stock-recruit steepness parameters. We compare the assessment results under multiple model hypotheses and data scenarios. Integrated growth model/first assessments of many stocks/... Results...
output: 
  pdf_document:
...
Fisheries stock assessment is concerned with determining the status of exploited fish stocks under uncertainty [@hilborn1992quantitative]. Stock assessment models are fit to multiple sources of data, including abundance or biomass indices, age compositions, and length composition data. 
...
Data limitations - modern fisheries management usually requires age structured models, age data is often missing as it is costly to collect.
...
- Growth model - this is now outside the model, is there room to add it and account for selectivity in the observations??
    ...
    - Random-at-length, using either eqbm age structure outside the model, or the modeled age structure and selectivity inside the model
...
Multifleet model: one fleet for each survey leg in Synoptic, one for trawl. Given the Synoptic survey is using the same vessel/gear across years and legs, we fix the selectivity across fleets for this set.

- There is sexual dimorphism, but I don't think we'll include it right now
